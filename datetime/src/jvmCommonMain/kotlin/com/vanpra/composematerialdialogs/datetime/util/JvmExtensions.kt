...
internal actual fun Month.getShortLocalName(locale: Locale): String =
    this.toJavaMonth().getDisplayName(TextStyle.SHORT_STANDALONE, locale.toPlatform())

internal actual fun Month.getFullLocalName(locale: Locale) =
    this.toJavaMonth().getDisplayName(TextStyle.FULL_STANDALONE, locale.toPlatform())

internal actual fun DayOfWeek.getShortLocalName(locale: Locale) =
    this.toJavaDayOfWeek().getDisplayName(TextStyle.SHORT_STANDALONE, locale.toPlatform())

internal actual fun Month.testLength(year: Int, isLeapYear: Boolean): Int = this.toJavaMonth().length(isLeapYear)

internal actual operator fun DayOfWeek.plus(days: Long): DayOfWeek =
    toJavaDayOfWeek().plus(days).toKotlinDayOfWeek()

internal actual fun DayOfWeek.getNarrowDisplayName(locale: Locale): String = toJavaDayOfWeek().getDisplayName(TextStyle.NARROW, locale.toPlatform())